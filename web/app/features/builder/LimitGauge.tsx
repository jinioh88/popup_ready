    ...
    <Card as="section" className="flex flex-col gap-4" aria-label="배치 한도">
      ...
      <div className="flex flex-col gap-2">
        ...
        <Meter
          ...
          thickness="h-1"
          /*
            **면적 축은 어느 상태에서도 채도를 갖지 않는다** — 스타일가이드 §8 「색을 갖는 축은
            게이트뿐이다」(2026-08-25 PM 결정, C-2 인수 발견).

            전에는 혼잡일 때 `bg-warning`이었는데, **전력의 "한도 임박"도 같은 `bg-warning`**이라
            혼동이 가장 위험한 경고 상태에서 두 축이 같아 보였다. 사용자 답변:
            *"굵기가 달라 성격이 달라보이지만, 색상을 다르게 하면 더 다르게 느껴질듯."*

            고칠 것은 "면적의 색"이 아니라 **"면적이 색을 갖는다는 것" 자체**다. 면적에 다른
            채도색을 주면 사용자는 *면적에도 상태가 있다*고 학습하고, 그것이 §2.2-F가 없애려던
            인식이다. 빼는 쪽이 규칙을 만든다 — **"색이 있으면 잠긴다"는 학습되지만
            "빨강과 주황은 뜻이 다르다"는 학습되지 않는다.**

            혼잡은 아래 뱃지와 문구가 전달하므로 정보 손실이 없다(색 단독 전달 금지 규칙상
            텍스트는 어차피 필수다).
          */
          fill="bg-text-muted"
          label="도면 점유"
